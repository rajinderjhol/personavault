...
export default class ErrorHandler {
     ...
     * @param {number} timeout - Optional timeout (in milliseconds) to automatically hide the success message.
     */
    displaySuccess(message, timeout = 5000) {
        ...
        errorMessageContainer.textContent = message;
        errorMessageContainer.style.display = 'block';
        errorMessageContainer.style.color = 'green'; // Success messages are displayed in green

        // Automatically hide the success message after the specified timeout
        if (timeout > 0) {
            setTimeout(() => {
                this.clearError();
            }, timeout);
